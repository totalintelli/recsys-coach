...
| 2026-06-23 | EDA 파이프라인 재설계·상위 표 10개 제한, README 작성 | `src/eda.py`, `app.py`, `README.md`, `requirements.txt` | EDA 리포트 가독성 개선 + 프로젝트 문서화 (`3b4eb3a`) |
| 2026-06-23 | RAG 파이프라인 확장: BM25+FAISS 하이브리드 검색, Cross-Encoder reranking, 다중 LLM 백엔드(upstage/llama3/qwen), 응답 캐시, 청크 출처 헤더 | `src/qa_chain.py` | 검색 품질 개선 + 로컬 LLM 지원 + API 크레딧 절감 |
| 2026-06-23 | Streamlit 업로드 제한(200MB) 설정 추가, `.gitignore` 정리 | `.streamlit/config.toml`, `.gitignore` | 서버/UI 업로드 한도 동기화, OS·계획 파일 제외 (`3c2e672`) |
| 2026-06-23 | EDA 진행 표시를 □/✓ 체크리스트로 전환, MessageSizeError 해결, ID성 컬럼 분류 | `src/eda.py`, `app.py` | 진행률 직관화 + 6750만 행 차트 전송 한도 초과 방지 + ID 컬럼 분석 적합화 |
| 2026-06-23 | GPU EDA 환경 복구 절차 문서화 | `requirements.txt` | CUDA 12.2 드라이버 호환 cudf/cupy/numba 버전 통일, CUDA_ERROR_INVALID_IMAGE 해결 |
